...
struct AddPriceRuleView: View {
    ...
    var body: some View {
        NavigationView {
            VStack(spacing: 20) {
                ...
                Button(action: {
                    ...
                    Text(((self.selectedPriceRule) != nil) ? "Save Price Rule Changes" : "Create Price Rule")
        ...
        .onAppear {
            print("🔍 selectedPriceRule title: \(selectedPriceRule?.title ?? "nil")")
    ...
    private func fillPriceRuleData() {
        guard let rule = selectedPriceRule else { return }
        ...
        if let endsAtString = rule.endsAt,
           let date = ISO8601DateFormatter().date(from: endsAtString) {
            endDate = date
        }
        
        useCodeOncePerCustomer = rule.oncePerCustomer ?? false
        usageLimit = String(rule.usageLimit ?? 0)
        
        allocationMethod = rule.allocationMethod
        valueType = rule.valueType ?? "percentage"
        
        print("🔍 After assignment:")
        print("   - valueType State: '\(valueType)'")
        print("   - allocationMethod State: '\(allocationMethod)'")
    }
